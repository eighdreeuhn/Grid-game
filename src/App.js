...
function App () {
  ...
  const generateBoard = () => {
    ...
    isSolvable(newBoard) ? setTiles(newBoard) : generateBoard()
  }
  ...
  const handleSwitch = e => {
    const activeIndex = tiles.findIndex(
      tile => tile.number === parseInt(e.target.id)
    )
    ...
    if (activeTile.number !== 'blank' && activeTile.row === blankTile.row) {
      activeTile.column - blankTile.column < 0
        ? (direction = -1)
        : (direction = 1)
      rowSwitch(activeIndex, blankIndex, direction)
    } else if (
      activeTile.number !== 'blank' &&
      activeTile.column === blankTile.column
    ) {
